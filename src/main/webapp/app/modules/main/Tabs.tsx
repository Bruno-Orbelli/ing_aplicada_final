...
const MyTabs = () => {
  ...
  return (
    <>
              ...
              <div className="row justify-content-center">
                {foodItems.map(item => (
                  <div className="col-md-3 my-2 mx-5" key={item.name}>
                    <OrderItemCard name={item.name} />
                  </div>
                ))}
              </div>
              <div className="d-flex justify-content-center mt-3">
                <Button color="success" id="confirmationButton" data-cy="confirmOrderButton" onClick={handleModal}>
                  Confirm order
                </Button>
              </div>
            </div>
          ...
          <Tab eventKey="stock" title="Add stock">
            <div id="underline-stock" className="tab-pane fade show active" role="tabpanel">
              <div className="row justify-content-center">
                {foodItems.map(item => (
                  <div className="col-md-3 my-2 mx-5" key={item.name}>
                    <StockItemCard name={item.name} />
                  </div>
                ))}
              </div>
              <div className="d-flex justify-content-center mt-3">
                <Button color="success" id="confirmationButton" data-cy="confirmStockButton" onClick={handleModal}>
                  Confirm restocking
                </Button>
              </div>
            </div>
      ...
      {activeTab === 'new-order' && (
        <OrderConfirmationModal items={foodItems.filter(item => item.selectedCounter > 0)} getOrders={getOrderItems} />
      )}
      {activeTab === 'current-orders' && selectedOrder && <OrderInformationModal />}
      {activeTab === 'stock' && (
        <StockConfirmationModal items={foodItems.filter(item => item.stockCounter > 0)} updateStock={updateStock} />
      )}
    </>
  );
};
